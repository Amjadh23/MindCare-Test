...
class _ReportListScreenState extends State<ReportListScreen>
  ...
  Future<void> _loadUserAssessments() async {
    try {
      // Get ALL users instead of just current user
      final usersSnapshot = await _firestore.collection('users').get();

      final List<Map<String, dynamic>> allAttempts = [];
      ...
      for (final userDoc in usersSnapshot.docs) {
        ...
        if (userData.containsKey('assessmentAttempts')) {
          final attempts = userData['assessmentAttempts'] as List<dynamic>;
      ...
      if (mounted) {
        setState(() {
          assessmentAttempts = allAttempts;
          isLoading = false;
        });
      }
    } catch (e) {
      print('Error loading all user assessments: $e');
      if (mounted) {
        setState(() {
          isLoading = false;
        });
                  ...
                  children: [
                    Column(
                      ...
                      children: [
                        ...
                        Text(
                          'View all user reports (${assessmentAttempts.length} total)',
                          style: TextStyle(
                            fontSize: 14,
